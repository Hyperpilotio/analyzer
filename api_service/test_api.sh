...
# create app
APP_ID=$(curl -s  -H  "Content-Type: application/json" -X POST --data-binary "@workloads/tech-demo-app.json" localhost:5000/api/apps | grep data | cut -d " " -f4  | cut -d "\"" -f2) ; echo $APP_ID
...
curl -H "Content-Type: application/json" -X POST localhost:5000/api/apps/$APP_ID/services --data-binary "@workloads/services.json"
