...
class BayesianOptimizerSession():
    ...
    def get_candidates(self, request_body, **kwargs):
        ...
        new_sample_dataframe = self.create_sample_dataframe(request_body)
        ...
        training_data_list = [BayesianOptimizerSession.make_optimizer_training_data(
            self.sample_dataframe, objective_type=o) for o in ['perf_over_cost',
    ...
    def check_termination(self):
        ...
        new_opt_poc = BayesianOptimizerSession.compute_optimum(all_samples)
    ...
    def make_optimizer_training_data(self, objective_type=None):
        ...
        feature_mat = np.array(self.sample_dataframe['feature'].tolist())
        ...
        # Compute objectives
        perf_over_cost = (
            perf_arr / self.sample_dataframe['cost']).rename("performance over cost")
    ...
    def create_sample_dataframe(self, request_body):
    ...
    def store_sizing_run(self):
        ...
        results = []
        for sample in self.sample_dataframe:
        ...
        sizing_run = {'run': len(sizing_runs) + 1,
                      'samples': len(self.sample_dataframe),
                      'results': results
                      }
